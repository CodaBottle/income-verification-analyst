...
const analyzeAttempts = new Map<string, { count: number; resetAt: number }>();
const globalApiAttempts = new Map<string, { count: number; resetAt: number }>();
...
setInterval(() => {
  ...
  for (const [key, entry] of analyzeAttempts) {
    if (now > entry.resetAt) analyzeAttempts.delete(key);
  }
  for (const [key, entry] of globalApiAttempts) {
    if (now > entry.resetAt) globalApiAttempts.delete(key);
  }
}, 5 * 60 * 1000);
...
const FEDERAL_POVERTY_LEVELS: { [key: number]: number } = {
